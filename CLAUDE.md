...
### Project cards
`ProjectCards.tsx` shows static cover images only — the owner removed the hover-to-play video previews (and the old `/test` prototype + `.mp4` assets), do not reintroduce them. Every cover follows one recipe (Sept 2026): a 1440px-wide capture of the project's live homepage cropped below its own header, Gaussian-blurred (sigma ≈ 4.5 at 720×496), tinted (white for light sites, navy for dark), with the project's logo composited in the centre; the untouched captures are not kept, so rebuild with `sharp` from a fresh capture. Each card links to the project's live site (`links[]`); a project with exactly two links (SWAYAM Plus: Website + LMS) renders `SplitCover` — one image with two hover zones that widen on hover/focus; it takes two full-size covers (`split_covers`: website, LMS) each clipped to its zone with `clip-path`, so hovering a half reveals more of that site, plus a separate `logo` overlay. Tags describe the organisation's service (e.g. "Platform Build", "Video Production"), never portfolio/skill words like "Full-Stack". Order: SWAYAM Plus, ChitraVaani, Smart Sutra, then the rest; four columns at `lg`. The "Take a sneak peek" panel reveals its device mockups via scroll progress (`useScroll` on the panel), not hover.
...
- `Hourglass3D.tsx` — the Why-Us hourglass. `/public/hourglass/hourglass.glb` (meshopt-compressed, decoded with `three/addons/libs/meshopt_decoder`) is the vessel; the sand is a live position-based granular simulation of 3,000 grains, each drawn as one camera-facing shaded sphere in a single instanced draw (no shadow maps, no glow lights; gravity is scaled up to ~28 units/s² so falls read at the vessel's scale) in `src/components/hourglass/` (`HourglassView.ts` owns renderer/scene/interaction, `physics.ts` the grains and the metered neck, `sandVisual.ts` the single instanced draw, `transformation.ts` the chamber-role state machine, `sand.ts` the vessel wall profile that must match the GLB). It runs a 32-second metered cycle, waits for the pile to settle, flips itself and repeats. Drag sideways to tilt (gravity follows), drag vertically to lift (grains get a kick); the vessel stays where it is released (the owner does not want it to snap back), and resting it inverted swaps the roles. Lazy-loaded when the section approaches and fades in on its first frame; there is no static stand-in any more (the owner removed it; `/public/hourglass.webp` is now unused). Without WebGL2, with reduced motion, on failure, or below `lg`, the section is plain stacked copy with no visual. Physics only steps while the section is in view. All sand is one golden colour (the owner rejected the raw-to-gold transition). Layout: the vessel is centered in the section with the copy split into two columns around it. The copy is laid out line by line with **pretext** (`@chenglou/pretext`, `prepareWithSegments` + `layoutNextLineRange`): `HourglassView.onOutline` projects the frame's bounding cylinder through the camera and convex-hulls it, `src/lib/hourglassSilhouette.ts` provides the analytic upright hull until the first frame plus `outlineExtent()`, and `WhyUs.tsx` gives every line only the width the hull leaves free at that height (`intrusion()`), so the text reflows as the hourglass tilts or lifts; the Get in touch button is inset the same way. Font and line height are read from the column's computed style and re-measured after `document.fonts.ready` (pretext's cache is cleared). The owner asked for pretext over the earlier CSS `shape-outside` floats (Sept 2026) — do not go back to floats. No card, no text overlay (the owner rejected both). The 3D vessel mounts only at `lg` and up (`useIsDesktop`). The left column sits a little above the neck and the right column starts lower (`LEFT_RISE` / `RIGHT_STAGGER`).
